...
function Table({ _orig, _data, slug, allMaterials }) {
        ...
        {stationToggles[`toggle${parentIndex}-${index}`] &&
          _data.pillars.map((pillar, pillarIndex) => {
            ...
            function handleChangedPillar(changedPillar) {
              let newData = { ...data };
              newData.routes[parentIndex].stations[index].pillars[
                pillarIndex
              ].name = changedPillar;
              setData(newData);
            }
            ...
            function handleDeletedPillar() {
              let newData = { ...data };
              newData.routes[parentIndex].stations[index].pillars.splice(
                pillarIndex,
                1
              );
              setData(newData);
            ...
            return (
              <>
                <tr>
                  ...
                  {matTextList.map((matText) => {
                    return (
                      <ValueTd matText={matText} pillarMaterials={pillar} />
                    );
                  })}
                </tr>
              </>
  ...
  const SumTr = ({ _data, mode, children }) => {
    ...
    return (
      <tr>
        <td
          className={`${children === "Đã xong" ? "bg-[#6dcacf]" : "bg-[#ffcb8b]"
            } font-bold text-black p-2 text-center`}
        >
          {children}
        </td>
        <td
          className={`${children === "Đã xong" ? "bg-white" : "bg-[#ffcb8b]"}`}
        >
          <ValueBox
            canEdit={false}
            value={children === "Tổng cộng" && sumArr.distance}
          />
        </td>
        <td
          className={`${children === "Đã xong" ? "bg-white" : "bg-[#ffcb8b]"}`}
        >
          <ValueBox
            canEdit={false}
            value={children === "Tổng cộng" && sumArr.middleLine}
          />
        </td>
        <td
          className={`${children === "Đã xong" ? "bg-white" : "bg-[#ffcb8b]"}`}
        >
          <ValueBox
            canEdit={false}
            value={children === "Tổng cộng" && sumArr.lowLine}
          />
        </td>
        {matTextList.map((matText) => {
          return (
              ...
              if (mats[i].detail !== null && matID === mats[i].detail._id) {
                value = (
                  <td
                    ...
                    className={mats[i].isDone && (mats[i].locked ? "bg-[#4BE383]/75" : "bg-orange-300")}
                    onClick={() => {
                      if (
                        mats[i].locked !== true &&
                        editing === false &&
      ...
      <Flex className="flex justify-between items-center mb-3 z-50">
        ...
        {!exportMode ?
          <div className="flex gap-2">
            {!editing && (
              <Button onClick={() => setEditing(true)} leftIcon={<HiWrenchScrewdriver />}>Chỉnh sửa</Button>
            )}
            ...
            {!editing && <Button onClick={() => setExportMode(true)} leftIcon={<FaFileExport />}>
              {"Xuất biên bản"}
            </Button>}
            <Popover placement="left">
              {({ isOpen, onClose }) => (
